describe("Casas de Consolidación", () => {
  beforeEach(() => {
    ...
    it("Crear casa de consolidación Shekinah debe dar error", () => {
    cy.visit("/admin/consolidationHouses");
    cy.contains("Agregar CDC").click();
    cy.get('input[placeholder="Ingrese el nombre de la CDC"]').type("Shekinah");
    ...
    cy.get('[role="status"][aria-live="polite"]').should(
     ...
     it("Crear casa de consolidación Apostol", () => {
    cy.visit("/admin/consolidationHouses");
    cy.contains("Agregar CDC").click();
    cy.get('input[placeholder="Ingrese el nombre de la CDC"]').type("Apostol");
    // Selecciona el valor del líder en el dropdown
    cy.get("select#leader").select("Luz"); // Puedes ajustar el valor según tus necesidades
    // Rellena el input de Fecha
  ...
  it("Editar casa de consolidación", () => {
    cy.visit("/admin/consolidationHouses");
  ...
  it("Eliminar casa de consolidación", () => {
    cy.visit("/admin/consolidationHouses");
    // Hacer clic en el botón "Editar" del primer elemento (index 0)
    cy.get(

export const AnnouncementCard = ({ announcement }) => {
    ...
    return(
        ...
        <article className="w-full sm:w-1/2 border-solid border border-primary rounded-lg overflow-hidden h-72">
          <p className="text-tertiary font-sans line-clamp-6 ml-2 sm:line-clamp-8 sm:text-base md:text-lg lg:text-xl">
            {announcement.description}
          </p>
          <h2 className="text-2xl ml-2 font-semibold text-tertiary mt-2 uppercase font-sans">
            {announcement.name}
          </h2>
          <p className="text-tertiary font-sans ml-2 font-semibold"> Fecha: {formatearFecha(announcement.date)}</p>
          <p className="text-tertiary font-sans ml-2 font-semibold"> Hora: {new Date(announcement.date).toISOString().split("T")[1].split(".")[0].slice(0, -3)}</p>
        </article>

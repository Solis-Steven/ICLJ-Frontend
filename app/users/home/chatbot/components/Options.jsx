...
const Options = (props) => {
  const options = [
    ...
    { text: "Pastor", handler: props.actionProvider.PastoresHandler, id: 3 },
    { text: "Ubicación", handler: props.actionProvider.UbicacionHandler, id: 4 },
  ];

  const buttonsMarkup = options.map((option) => (
    <button key={option.id} onClick={option.handler} className=" px-4 mx-2 my-2 bg-transparent text-center border-2 rounded-lg border-primary justify-items-center">
      {option.text}
    </button>
  ));
